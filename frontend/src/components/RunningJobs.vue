<template>
    <div>
        <div class="flex items-center justify-between mb-4 px-2">
            ...
            <div class="flex items-center gap-3">
                <span v-if="lastUpdated" class="text-xs text-base-content/50"> Updated {{ lastUpdatedLabel }} </span>
                <button class="btn btn-sm btn-ghost" :disabled="loading" @click="fetchJobs">
        ...
        <div v-else-if="jobs.length === 0" class="text-center py-12 text-base-content/40">No jobs currently running.</div>
...
function formatElapsed(startedAt: string): string {
    const elapsed = Math.max(0, Math.floor((Date.now() - toUtcDate(startedAt).getTime()) / 1000));
